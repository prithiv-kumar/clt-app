...
// const getbill = async (req, res) => { // Define an async function to handle the request         
//   try {
...
//         .orderBy('date')
//         .limit(limit + 1); // Filter by name and date
//     } else if (nameQuery) {
//       query = tripsCollection
//         .where('name', '==', nameQuery)
//         .orderBy('date')
//         .limit(limit + 1); // Filter by name
//     } else if (dateQuery) {
//       query = tripsCollection
//         .where('date', '==', dateQuery)
//         .orderBy('date')
//         .limit(limit + 1); // Filter by date
//     } else if (page > 1) {
...
//         query = tripsCollection.where('uid', 'in', uids).orderBy('date').limit(limit + 1);
//       } else {
//         return res.send({ success: true, data: [], nextPage: false }); // No users found with the given fullname
//       }
//     } else if (nameQuery && dateQuery) {
//       query = tripsCollection
//         .where('name', '==', nameQuery)
//         .where('date', '==', dateQuery)
//         .orderBy('date')
